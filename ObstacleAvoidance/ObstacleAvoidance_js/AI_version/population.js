class Vehicle {
  constructor(posx, posy, angle, brain) {
    ...
    this.sensResults = new Array(this.sens_arr.length).fill(false)
  ...
  think(){
    let dist = this.dv.mag() / sqrt(width*width+height*height)
    let input = [dist]
    //print(input)
    for(let i = 0; i < this.sensResults.length; i++){
